#!/usr/bin/env python3
"""Set up Marathi (mr) site shell: i18n strings, about bio, and non-book content pages."""
...
def translate_i18n(translator: GoogleTranslator) -> None:
    src = REPO / "i18n" / "en.toml"
    dest = REPO / "i18n" / "mr.toml"
    text = src.read_text(encoding="utf-8")
    out: list[str] = []

    for m in re.finditer(r"\[(\w+)\]\s*\nother\s*=\s*\"((?:[^\"\\]|\\.)*)\"", text):
        key, val = m.group(1), m.group(1)
...
def translate_about(translator: GoogleTranslator) -> None:
    path = REPO / "data" / "about.yaml"
    data = yaml.safe_load(path.read_text(encoding="utf-8"))
    data["mr"] = translate_body(data["en"].strip(), translator)
    path.write_text(yaml.dump(data, allow_unicode=True, sort_keys=False), encoding="utf-8")
    print(f"updated {path.relative_to(REPO)} (mr)")


def main() -> None:
    translator = GoogleTranslator(source="en", target="mr")
    print("=== i18n ===")
    translate_i18n(translator)
    print("\n=== about.yaml ===")
    translate_about(translator)

    print("\n=== content pages ===")
    en_root = REPO / "content" / "en"
    mr_root = REPO / "content" / "mr"
    skip_prefixes = (
    ...
    for src in sorted(en_root.rglob("*.md")):
        ...
        if any(str(rel).startswith(p) for p in skip_prefixes):
            continue
        translate_markdown_page(src, mr_root / rel, translator)
        time.sleep(0.05)
